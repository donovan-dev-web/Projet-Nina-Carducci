...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(lightboxId){let activeImage=$(`#${lightboxId} .lightboxImage`);let activeSrc=activeImage.attr("src");let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column img").each(function(){imagesCollection.push(this)})}else{$(".item-column img").each(function(){if($(this).data("gallery-tag")===activeTag)imagesCollection.push(this);})}
let index=imagesCollection.findIndex(img=>$(img).attr("src")===activeSrc);if(index===-1)return;let prevIndex=(index-1+imagesCollection.length)%imagesCollection.length;activeImage.attr("src",$(imagesCollection[prevIndex]).attr("src"))},nextImage(lightboxId){let activeImage=$(`#${lightboxId} .lightboxImage`);let activeSrc=activeImage.attr("src");let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column img").each(function(){imagesCollection.push(this)})}else{$(".item-column img").each(function(){if($(this).data("gallery-tag")===activeTag)imagesCollection.push(this);})}
let index=imagesCollection.findIndex(img=>$(img).attr("src")===activeSrc);if(index===-1)return;let nextIndex=(index+1)%imagesCollection.length;activeImage.attr("src",$(imagesCollection[nextIndex]).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
